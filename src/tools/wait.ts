...
import type { WhatsmeowClient, StoredMessage } from '../whatsapp/client.js';
...
export function registerWaitTools(
  ...
  server.tool(
    ...
    async ({ timeout, chat, from_phone }) => {
      ...
      let chatJidFilter = null as string | null;
      if (chat) {
      ...
      let senderFilter = null as string | null;
      if (from_phone) {
      ...
      const msg = await new Promise<StoredMessage | null>((resolve) => {
        const timer = setTimeout(() => {
          ...
          const waClientWithWaiters = waClient as unknown as { _messageWaiters?: MessageWaiter[] };
        ...
        const waClientWithAddWaiter = waClient as unknown as { addMessageWaiter?: (f: typeof filter, r: typeof wrappedResolve) => void };
