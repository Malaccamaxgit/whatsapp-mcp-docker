...
function authEnvLinkTimeoutSec() {
...
function authEnvPollIntervalSec() {
...
function appendWaitResult(text, wait) {
  if (wait.ok) {
...
export function registerAuthTools(server, waClient, permissions, audit) {
  server.tool(
    'disconnect',
    'Log out and disconnect from WhatsApp. This clears the session and requires re-authentication. Use this when you want to unlink the current device or switch to a different WhatsApp account.',
    {},
    async () => {
      ...
      try {
        ...
        audit.log('disconnect', 'logout_failed', { error: error.message }, false);
        return {
          content: [
            {
              type: 'text',
  ...
  server.tool(
    'authenticate',
    'Link this device to WhatsApp. Returns an 8-digit pairing code or a QR code. By default, waits and polls until the device links or a timeout, then appends success or failure to the same response. Defaults for waitForLink, linkTimeoutSec, and pollIntervalSec come from Docker MCP Toolkit profile (whatsapp-mcp-docker.auth_* config → AUTH_* env) when arguments are omitted; pass explicit tool arguments to override per call.',
    {
      phoneNumber: z
        .string()
        .describe(
    ...
    async ({ phoneNumber, waitForLink, linkTimeoutSec, pollIntervalSec }) => {
      ...
      if (!phoneNumber && waClient.hasSession) {
        ...
        try {
          ...
          console.error('[AUTH] Reconnect attempt failed:', err.message);
        }
      ...
      try {
        ...
        const backoffNote =
          permissions._authBackoffSec > 0
            ? `\n\nNote: If this code expires unused, you can retry after ${permissions._authBackoffSec}s cooldown.`
            : '';
        ...
        const nextRetry = permissions._authBackoffSec;
        audit.log(
          'authenticate',
          'pairing_failed',
          {
