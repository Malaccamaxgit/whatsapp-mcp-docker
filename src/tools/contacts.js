...
export function registerContactTools(server, waClient, store, permissions, audit) {
  // ── get_user_info ─────────────────────────────────────────────
  ...
  server.tool(
    ...
    async ({ phones }) => {
      ...
      try {
        ...
        return { content: [{ type: 'text', text: `Failed to get user info: ${err.message}` }], isError: true };
      }
    },
  ...
  server.tool(
    ...
    async ({ phones }) => {
      ...
      try {
        ...
        return { content: [{ type: 'text', text: `Failed to check: ${err.message}` }], isError: true };
      }
    },
    { annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: true } }
  );
    ...
    async ({ target }) => {
      ...
      try {
        ...
        const url = typeof result === 'string'
          ? result
          : result?.url || result?.URL || result?.profilePictureURL || null;
        ...
        return { content: [{ type: 'text', text: `Failed to get profile picture: ${err.message}` }], isError: true };
      }
    },
    { annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: true } }
  );
