...
describe('validatePhoneNumber', () => {
  ...
  it('rejects null/undefined/empty', () => {
    assert.equal(validatePhoneNumber(null as unknown as string).valid, false);
    assert.equal(validatePhoneNumber(undefined as unknown as string).valid, false);
    assert.equal(validatePhoneNumber('').valid, false);
    assert.equal(validatePhoneNumber('   ').valid, false);
  });

  it('rejects non-string input', () => {
    assert.equal(validatePhoneNumber(12345 as unknown as string).valid, false);
  });
...
describe('toJid', () => {
  ...
  it('returns null for falsy input', () => {
    assert.equal(toJid(null as unknown as string), null);
    assert.equal(toJid(undefined as unknown as string), null);
...
describe('isJid', () => {
  ...
  it('rejects non-JIDs', () => {
    ...
    assert.equal(isJid(null as unknown as string), false);
    assert.equal(isJid(42 as unknown as string), false);
  });
});
...
describe('isGroupJid', () => {
  ...
  it('returns false for non-strings', () => {
    assert.equal(isGroupJid(null as unknown as string), false);
    assert.equal(isGroupJid(123 as unknown as string), false);
  });
});
